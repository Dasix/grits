	...
	"C2C.dustjs.Renderer", {
		...
		members : /** @lends C2C.dustjs.Renderer **/ {
			...
			_copyStaticContent: function() {
				...
				return fsl.scanForFiles( paths ).then(
					...
					function afterStaticContentScan( result ) {
						...
						return Promise.all(staticCopyOps);
			...
			_storeData: function( dataPath, data ) {
				...
				// We use `lodash`'s `mergeWith` function, instead of simple
				// `merge` so that we can force arrays to be merged, which
				// is not the default behavior of `merge`.
				me.$$contextData = _.mergeWith( me.$$contextData, newData,

					function( objValue, srcValue, key, object, source, stack ) {

						// If the two values are arrays, we 'concat' the values.
						if( tipe( objValue ) === "array" && tipe( srcValue ) === "array" ) {
