...
qx.Class.define(
	"Dasix.grits.ResourcePathCollection", {
		...
		members : /** @lends Dasix.grits.ResourcePathCollection **/ {
			 ...
			 * @returns {void}
			 */
			addScanExtension: function( extension ) {

				// Locals
				var me = this;
				var cfg = me.getConfig();
				var existing = me.getScanExtensions();

				// If no extensions exist..
				if( existing === null ) {
					existing = [];
				}
				...
				// Make the extensions sorted and unique
				existing = _.sortBy( existing );
				existing = _.uniq( existing );
				
				// Store the new setting
				cfg.scanExtensions = existing;
			...
			clearPaths: function( force ) {
				...
				var me = this;
				var newPaths = [];
				var existing = me.getPaths();
				var existingCount, logMessage;
				...
				if( !force ) {
					logMessage = "Cleared '" + me.getName() + "' resource paths";
					_.each( existing, function( path ) {
					...
					logMessage = "Cleared ALL '" + me.getName() + "' resource paths";
				}

				// Log the operation
				if( existingCount > 0 ) {
					var diff = existingCount - newPaths.length;
					me.log( "debug", "clear", logMessage + " (removed " + diff + " of " + existingCount + " paths)" );
				}
			...
			_addPathObject: function( resourcePath ) {
				...
				if( !me.hasPath( resourcePath.getAbsolutePath() ) ) {
					...
					// Log the addition
					me.log("debug", "add", "  -> Adding '" + me.getName() + "' Path: '" + resourcePath.getAbsolutePath() + "'" );
			...
			/**
			 * Gets information about scanning all resource paths in this collection.
			 *
			 * @instance
			 * @access public
			 * @param opts
			...
			resolveScanSettings: function( opts ) {
				...
				_.each( paths, function( path ) {
					...
					var oneSet = path.resolveScanSettings( opts );
					_.each( oneSet, function( oneExt ) {
						ret.push( oneExt );
					});
			...
			/**
			 * Gets the appropriate globs for scanning all resource paths in this collection.
			 *
			 * @instance
			 * @access public
			 * @param opts
			 * @returns {object[]}
			 */
			resolveScanGlobs: function( opts ) {

				// Locals
				var me 				= this;
				var scanSettings 	= me.resolveScanSettings( opts );
				var ret 			= [];

				// Scan each path
				_.each( scanSettings, function( ss ) {

					var glob = ss.path + "/" + ss.glob;
					ret.push( glob );
			 ...
			 * if the file exists within one of the {@link Dasix.grits.ResourcePath}
			 * objects in this collection.  If the file does not match any resource
			 * paths in this collection, however, then NULL is returned.
			 *
			 * @instance
			 * @access public
			...
			resolveFileObject: function( absolutePath ) {
				...
				// Check for a match within each path
				_.each( paths, function( path ) {

					if( ret === null ) {

						var subResult = path.resolveFileObject( absolutePath );
						if( subResult !== null ) {
							ret = subResult;
						}
			...
			eachResource: function( fn, opts ) {
				...
				// Default opts.noMatchHandler
				if( opts.noMatchHandler === undefined || opts.noMatchHandler === null || tipe( opts.noMatchHandler ) !== "function" ) {
					opts.noMatchHandler = null;
				}
				...
				return me.scan( opts ).then(
					...
					function onResourcesScanned ( result ) {
						...
						// If no results were returned and we were passed a "noMatchHandler" function,
						// then we will call it here.
						if( opts.noMatchHandler !== null && result.result.count === 0 ) {
							opts.noMatchHandler( result );
						}
