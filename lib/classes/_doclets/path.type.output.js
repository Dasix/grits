/**
 * Adds one or more paths to the 'output' path configuration. This method is
 * a convenience alas because, internally, this method simply calls
 * {@link Dasix.grits.Renderer#addPathOfType} with the first param set to 'output'.
 *
 * Important Note: This method is automatically created by {@link Dasix.grits.HasPathConfig#_createPathTypeAddMethod}
 * and will, therefore, not be found in the source code.
 *
 * @method Dasix.grits.Renderer#addOutputPath
 * @instance
 * @access public
 * @param {string|string[]} newPath An absolute path, or an array of
 * paths, to be add to the 'output' path configuration.
 * @returns {void}
 */

/**
 * Clears any existing 'output' paths and adds one more into the
 * configuration.  This method is just convenient replacement
 * for calling {@link Dasix.grits.Renderer#clearOutputPaths} and
 * {@link Dasix.grits.Renderer#addOutputPath}.
 *
 * Important Note: This method is automatically created by {@link Dasix.grits.HasPathConfig#_createPathTypeSetMethod}
 * and will, therefore, not be found in the source code.
 *
 * @method Dasix.grits.Renderer#setOutputPath
 * @instance
 * @access public
 * @param {string|string[]} newPath A path, or an array of paths, to add
 * to the 'output' path configuration.
 * @returns {void}
 */

/**
 * Returns the number of 'output' paths currently stored in the path
 * configuration object. This method is a convenience alas because,
 * internally, this method simply calls {@link Dasix.grits.Renderer#countPathsOfType}
 * with the first param set to 'output'.
 *
 * Important Note: This method is automatically created by {@link Dasix.grits.HasPathConfig#_createPathTypeCountMethod}
 * and will, therefore, not be found in the source code.
 *
 * @method Dasix.grits.Renderer#countOutputPaths
 * @instance
 * @access public
 * @returns {number} The number of 'output' paths currently stored in the path
 * configuration object
 */

/**
 * Returns all 'output' paths currently defined in the renderer's path
 * configuration object. This method is a convenience alas because,
 * internally, this method simply calls {@link Dasix.grits.Renderer#getPathsOfType}
 * with the first param set to 'output'.
 *
 * Important Note: This method is automatically created by {@link Dasix.grits.HasPathConfig#_createPathTypeGetMethod}
 * and will, therefore, not be found in the source code.
 *
 * @method Dasix.grits.Renderer#getOutputPaths
 * @instance
 * @access public
 * @param {boolean} [returnAsObject=false] If TRUE, then a standard
 * javascript object will be returned instead of an array.
 * @returns {number} All 'output' paths currently stored in the path configuration object
 */

/**
 * Clears all 'output' paths from the path configuration. This method is
 * a convenience alas because, internally, this method simply calls
 * {@link Dasix.grits.Renderer#clearPathsOfType} with the first param set to 'output'.
 *
 * Important Note: This method is automatically created by {@link Dasix.grits.HasPathConfig#_createPathTypeClearMethod}
 * and will, therefore, not be found in the source code.
 *
 * @method Dasix.grits.Renderer#clearOutputPaths
 * @instance
 * @access public
 * @returns {void}
 */
