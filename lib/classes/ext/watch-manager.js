	...
	"Dasix.grits.ext.WatchManager", {
		...
		members : /** @lends Dasix.grits.ext.WatchManager **/ {
			...
			startWatching: function() {
				...
				// Fire an event
				me.emit( "onWatchStarted", { watchManager: me } );
			...
			unwatch: function() {
				...
				// Log it
				me.log("info", "stop", "Stopping the file system watcher");
				...
				// Fire an event
				me.emit( "onWatchEnded", { watchManager: me } );
			...
			_onWatcherReady: function() {
				...
				// Log it
				me.log( "info", "ready", "File system watcher is loaded and ready ..." );
			...
			_onWatchTrigger: function( eventName, absolutePath ) {
				...
				_.each( me.$$collections, function( colData, name ) {
					...
					if( fileObject !== null ) {
						...
						// Log the event
						me.log("debug", "trigger", "Collection Watch Event Triggered..");
						me.logObject({
				...
				_.each( me.$$indFiles, function( indFiles, indType ) {
					...
					_.each( indFiles, function( handler, fileAbsPath ) {
						...
						if( fileAbsPath === absolutePath ) {
							...
							// Log it
							me.log("debug", "trigger", "File Watch Event Triggered..");
							me.logObject({
