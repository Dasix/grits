	...
	"Dasix.grits.ext.DustManager", {
		...
		members : /** @lends Dasix.grits.ext.DustManager **/ {
			...
			_initDust: function() {
				...
				// Capture factory dust config
				me.setComponentFactoryConfig("dust", dust.config);

				// Apply Grits.js overrides to the Dust.js factory defaults
				me.setComponentDefaultConfig("dust", {
					whitespace: true
				});

				// Store a reference to Dust.js
				me.dust = dust;
			 ...
			 * called immediately before each Dust.js compilation and render op.
			 * This method will use the "Component Config Manager" to resolve a final
			 * configuration object for Dust.js.
			 *
			 * @instance
			 * @access private
			...
			_getDustConfigForPage : function( whiteMatter ) {
				...
				// Locals
				var me = this;
				var overrides = me._getConfigDataFromMatter( "dust", whiteMatter );

				// Return the config
				return me.getComponentConfig("dust", overrides);

			},

			/**
			 * Utility function that checks a white-matter generated object
			 * for component configuration data.
			 *
			 * @instance
			 * @access private
			 * @param {object} whiteMatter
			 * @param {string} componentName
			 * @returns {object}
			 */
			_getConfigDataFromMatter: function( componentName, whiteMatter ) {
				...
				if( !_.isNil( whiteMatter )
					...
					&& tipe( whiteMatter.data.config[ componentName ] ) === "object" ) {

					ret = whiteMatter.data.config[ componentName ];
			...
			renderString: function( sourceFilename, source, renderToFile ) {
				...
				if( grits.hasRendered === true ) {
					...
					me.log("debug", "Forcing prerequisite render() operation for renderString()");
					init = grits.render();
				}
			...
			compileAll : function() {
				...
				var scanOpts = {
					noMatchHandler: function() {
						me.log("info", "found.none", "No content source files were found or loaded!");
					}
				};
			...
			compileString : function( name, src, srcFile, parseFrontMatter ) {
				...
				me.log( "debug", "compile", "Compiling content..");
				me.logObject({
				...
				if( parseFrontMatter ) {
					...
					me.log( "debug", "check.fm", "  -> [ Checking template: '" + name + "' for front-matter ]" );
				...
				if( srcFile.is("md") ) {
					...
					// Log it
					me.log( "debug", "parse.markdown", "  -> [ Parsing Markdown for Template: '" + name + "'" + markdownNote + " ]" );
					...
					src = grits.markdownParser.parse( src, debugMarkdown,
						{
							marked: me._getConfigDataFromMatter( "marked", matterData ),
							hljs: me._getConfigDataFromMatter( "hljs", matterData )
						}
					);
			...
			renderOne: function( srcFile, renderToFile ) {
				...
				// Log the info we have so far
				me.log( "debug", "render", "Rendering content..");
				me.logObject({
						...
						me._doDustRender( templateName, context ).then(
							...
							function afterDustRender( renderedContent ) {
								...
								if( renderedContent !== null && renderToFile ) {
									...
									me.log( "info", "write", "Writing file: '" + destFilePath + "' (" + renderedContent.length + " bytes) ]" );
			...
			_doDustCompilation: function( templateName, source) {
				...
				me.log( "debug", "compile", "  -> [ Compiling template: '" + templateName + "' ]" );
				...
				try {
					...
					me.logError( new vError( errMessage ), true );
			...
			triggerRefUpdate: function( refType, refName ) {
				...
				// Log the ref call
				me.log( "debug", "ref.check", "Checking for content that references '" + refName + "' (" + refType + ")" );
				...
				_.each( me.$$refData, function( data, absolutePath ) {
					...
					if( refs[ refType ] !== undefined ) {
						...
						if( refs[ refType ][ refName ] !== undefined ) {
							...
							me.log("debug", "ref.found", "  -> Found reference in: '" + absolutePath + "'");
