		...
		members : /** @lends Dasix.grits.ext.DustManager **/ {
			...
			compileString : function( name, src, srcFile, parseFrontMatter ) {
				...
				if( parseFrontMatter ) {
					...
					matterData = grits._parseMatter( src, name );
				...
				if( srcFile.is("md") ) {
					...
					// Parse the markdown
					src = grits._parseMarkdown( src, debugMarkdown );
			...
			renderOne: function( srcFile, renderToFile ) {
				...
				// Define the context
				var context = grits._getContextForTemplate( templateName );
