	...
	"Dasix.grits.ext.DustManager", {
		...
		members : /** @lends Dasix.grits.ext.DustManager **/ {
			...
			compileString : function( name, src, srcFile, parseFrontMatter ) {
				...
				// Fire an event
				me.emit( "onDustCompile", { content: src, srcFile: srcFile, templateName: name, dustManager: me } );
					...
					renderOps.push(
						...
						me._doDustRender( templateName, context ).then(
							...
							function afterDustRender( renderedContent ) {
								...
								// Fire an event
								me.emit( "onDustRender", { content: renderedContent, srcFile: srcFile, destFile: destFile, dustManager: me } );
