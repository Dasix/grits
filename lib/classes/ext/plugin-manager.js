		...
		members : /** @lends Dasix.grits.ext.PluginManager **/ {
			...
			addPlugin : function( plugin ) {
				...
				if( tipe(plugin) === "string" ) {
					me.log("Loading plugin with require( '" + plugin + "' ) ...");
					vaguePluginName = "plugin ('" + plugin + "')";
					try {
						plugin = require(plugin);
					} catch( err ) {
						errMessage = "PluginManager.addPlugin() was passed a string ('" + plugin + "'), which is assumed to be a file path, but could not find a plugin at that location.";
				...
				if( tipe(plugin) !== "function" ) {
					errMessage = "PluginManager.addPlugin() was passed an invalid " + vaguePluginName + ".  #addPlugin() expects either a string path to a plugin module or a plugin constructor function but a/an '" + tipe(plugin) + "' was provided.";
				...
				if( initialized.pluginName === undefined || tipe(initialized.pluginName) !== "string" || initialized.pluginName.replace(/\s/g,"") === "" ) {
					errMessage = "PluginManager.addPlugin() was passed an invalid " + vaguePluginName + "; it does not have a .pluginName property (or it was blank), which is required for plugin identification.";
					me.logError( new Error( errMessage ) );
				}
