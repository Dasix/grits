	...
	"Dasix.grits.ext.ReloadServer", {
		...
		members : /** @lends Dasix.grits.ext.ReloadServer **/ {
			...
			_logConfig: function() {
				...
				me.log("LiveReloadX Server Config:");
			...
			start: function() {
				...
				if( outputs.length === 0 ) {
					me.log("Warning: The LiveReloadX server could not be started because no output directories were specified in the configuration.");
					return;
				}
				...
				// Log it
				me.log("Starting the LiveReloadX Server");
				me._logConfig();
			...
			stop: function() {
				...
				// Log it
				me.log("Stopping the LiveReloadX server");
			...
			_handleFileUpdate: function( abs, rel ) {
				...
				if( !me.$$notifyPending ) {
					...
					if( me.$$notifyDelay === 0 ) {
						...
						// Log it
						me.log( "Delaying LiveReloadX notifications for " + me.$$notifyDelay + "ms" );
			...
			_doLiveReloadNotify: function() {
				...
				// Log it
				me.log( "Sending [" + queue.length + "] update notifications to LiveReloadX" );
