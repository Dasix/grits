...
// Pretend that we're Electron so that `sass` will
// recognize it's running in a node environment.
process.versions['electron'] = 'pretending.to.be.electron';
...
renderSassAsync = Promise.promisify(sass.render);
	...
	"Dasix.grits.ext.SassRenderer", {
		...
		members : /** @lends Dasix.grits.ext.SassRenderer **/ {
			...
			_renderOne: function( srcFile ) {
				...
				// Render
				return renderSassAsync( sassRenderSettings ).then(
					function( res ) {
