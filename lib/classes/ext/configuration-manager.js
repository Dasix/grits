		...
		members : /** @lends Dasix.grits.ext.ConfigurationManager **/ {
			...
			getConfig : function() {
				...
				return {
					...
					attemptRecovery	: grits.getAttemptRecovery(),
			...
			_stepErrors : function( configData ) {
				...
				// If provided: Apply "attempt recovery" config
				if( configData.attemptRecovery !== undefined && configData.attemptRecovery !== null ) {
					if( configData.attemptRecovery === true ) {
						grits.setAttemptRecovery(true);
					} else {
						grits.setAttemptRecovery(false);
					}
				}
