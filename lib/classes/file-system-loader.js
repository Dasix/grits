		...
		members : /** @lends Dasix.grits.FileSystemLoader **/ {
			...
			scanForFiles: function( path, fileExtension ) {
				...
				return Promise.all( promises ).then(
					...
					function( allRes ) {
						me.log("debug", "fsl.scan", "All complete");
			...
			/**
			 * Gets a scan glob based on a file extension.
			 *
			 * @instance
			 * @access public
			 ...
			 * @returns {string}
			 */
			getScanGlob: function( fileExtension ) {
			...
			_doOneScan : function( path, fileExtension ) {
				...
				// Get glob
				glob = me.getScanGlob( fileExtension );

				// Name this scan
				scanName = path + "/" + glob;

				// Log the operation
				me.log("debug", "fsl.scan.start", "- " + scanName + " [START]" );
					...
					function( resolve, reject ) {
						...
						scan(
							...
							function( file ) {
								...
								// Append return
								me.log("debug", "fsl.scan.yield", "- " + scanName + " => " + file.name );
								file.scanPath = path;
								ret.push( file );
							...
							function() {
								me.log("debug", "fsl.scan.complete", "- " + scanName + " [COMPLETE]" );
								resolve( ret );
							}
						);
