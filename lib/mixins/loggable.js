...
qx.Mixin.define("Dasix.grits.Loggable", {
		...
		properties :  {
			...
			/**
			 * @var {boolean} Dasix.grits.Loggable.attemptRecovery Determines
			 * whether or not Grits will attempt to recovery from errors that
			 * it can recover from.
			 * @getter getAttemptRecovery
			 * @mutator setAttemptRecovery
			 */
			attemptRecovery: {
				init: false,
				apply: "_applyAttemptRecovery"
			}
		...
		members: /** @lends Dasix.grits.Loggable **/ {
			...
			//<editor-fold desc="+++++ Config: Attempt Error Recovery +++++">

			/**
			 * Apply handler for the `#attemptRecovery` property
			 *
			 * @access private
			 * @param {string?} val
			 * @param {string?} old
			 * @returns {void}
			 */
			_applyAttemptRecovery: function( val, old ) {

				// Locals
				var me = this;

				if( val === true ) {
					me.log("debug", "error.recovery", "Limited Error Recovery ENABLED");
				} else {
					me.log("debug", "error.recovery", "Limited Error Recovery DISABLED");
					val = false;
				}
			...
			_initLogger: function() {
				...
				if( format === "json" ) {
					...
					// We use a global object for the console output stream
					// as to avoid overloading process.stdout during unit tests
					if( global.$consoleStreamObj === undefined ) {

						streamLib = require("../util/log-formatters/console");
						streamObj = new streamLib();
						streamObj.pipe( process.stdout );

						global.$consoleStreamObj = streamObj;

					} else {

						streamObj = global.$consoleStreamObj;
			...
			/**
			 * Logs an error.  Depending on the mode grits is in (single render
			 * or 'watch') and the value passed to 'canRecover', the process
			 * may exit immediately after the error is displayed.
			 *
			 * @access public
			 ...
			 * @returns {void}
			 */
			logError: function( topic, err, canRecover ) {
				...
				// Determine if we should exit after the error
				if( canRecover === true && me.getAttemptRecovery() === true ) {
					return me.logWarning( topic, err );
				}
				...
				me.log( "debug", topic, "-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-" );
				me.log( "debug", topic, "" );
				me.log( "fatal", topic, "A CRITICAL ERROR HAS OCCURRED:" );
				me.log( "debug", topic, "" );

				me.log( "fatal", topic, err.message );
				me.log( "debug", topic, "" );
				...
				me.log( "fatal", "render.fail", "The Grits render operation has failed and will exit..");
				process.exit(1);
