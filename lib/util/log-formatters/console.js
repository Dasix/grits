/**
 * Log stream/formatter for Bunyan that outputs in human-readable format.
 * Forked from / Based on: https://www.npmjs.com/package/bunyan-prettystream
 *
...
function PrettyStream(opts){
	...
	function extractTime(rec){
		...
		return stylize(time.substr(11, 12));
	}
	...
	function extractLevel(rec){
		...
		return stylize(level, levelLabelColors[rec.level]);
	}
	...
	function extractMsg(rec){
		return stylize(rec.msg, messageColors[rec.level]);
	}
	...
	this.formatRecord = function formatRecord(rec){
		...
		*/
		...
		// Check for a topic
		if( rec.topic === undefined ) {
			topic = "";
		} else {
			topic = stylize( ' (' + rec.topic + ')', 'grey' );
		}

		// Show the message
		if( isEmpty ) {

			// We won't show the topic if the message is empty
			return format('[%s] %s: %s\n',
				time,
				level,
				msg);
			...
			// We show the topic if the message is not empty
